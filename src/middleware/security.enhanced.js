...
const sanitizeObject = obj => {
  ...
  for (const [key, value] of Object.entries(obj)) {
    if (typeof value === 'string') {
      ...
      if (key === 'email') {
        sanitized[key] = validator.isEmail(sanitized[key])
          ? validator.normalizeEmail(sanitized[key])
          : sanitized[key];
      }
    } else if (
      typeof value === 'object' &&
      ...
      sanitized[key] = value.map(item =>
        typeof item === 'string'
          ? xss(item, { whiteList: {}, stripIgnoreTag: true })
          : item
      );
    } else {
      sanitized[key] = value;
...
export const requestSizeLimit = (limit = '10mb') => {
  return (req, res, next) => {
    if (
      ...
      parseInt(req.headers['content-length']) > parseSize(limit)
    ) {
